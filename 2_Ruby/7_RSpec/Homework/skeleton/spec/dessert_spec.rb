...
describe Dessert do
  let(:chef) { double("chef", name: "Robin") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("pastry", "10", chef) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = %i(sugar, eggs, milf, flour)
  ...
  describe "#eat" do
    it "subtracts an amount from the quantity" do
      expect(dessert.eat(8)).to eq(2)
    end

    it "raises an error if the amount is greater than the quantity" do
      expect{ dessert.eat(12) }.to raise_error(/not enough/)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      ...
      expect(dessert.serve).to eq("Chef Robin the Great Baker has made 10 pastries!")
    end
  end
